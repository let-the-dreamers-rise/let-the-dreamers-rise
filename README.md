...
|---------|------------|----------|
| **[rein](https://github.com/let-the-dreamers-rise/rein)** | A smart account an autonomous agent can operate and cannot drain. The owner's spending policy is enforced by the chain, refusals are free view calls the agent can read before acting, and v2 compiles the policy from the agent's own on-chain trail. | Live with verified source on Base Sepolia and Whitechain Sepolia; a fully compromised agent tried six ways to empty it and lost nothing. v2, measured on the first run: 22 of 22 honest held-out calls allowed, 10 of 10 attacks refused. 46 tests, not audited. [Site](https://rein-nine.vercel.app). |
...
| **[p2mr-assurance-lab](https://github.com/let-the-dreamers-rise/p2mr-assurance-lab)** | Independent conformance testing for BIP 360 (P2MR, Bitcoin's post-quantum output proposal), and the first open measurement of what post-quantum signatures cost inside a Bitcoin transaction. | A from-scratch implementation cross-checked against the official reference on 13 official vectors and 2,000 random trees found four graded divergences, two of them real gaps in the reference. Post-quantum P2MR inputs cost 17x to 131x a Schnorr spend. Zero dependencies, fixtures pinned by SHA-256, CI. |
